...
app.post('/register', async (req, res) => {
  ...
  try {
    ...
    await executeQuery('INSERT INTO Users (username, password, created_at, updated_at) VALUES (?, ?, NOW(), NOW())', [
      username,
      password,
    ]);
...
app.post('/login', async (req, res) => {
  ...
  try {
    const user = await executeQuery('SELECT * FROM Users WHERE username = ? AND password = ?', [
      username,
      password,
    ]);
...
app.post('/edit-profile', async (req, res) => {
  ...
  try {
    const user = await executeQuery('SELECT * FROM Users WHERE id_user = ?', [id_user]);
    if (user.length === 0) {
      return res.status(404).json({ message: 'User not found!' });
    }

    if (newUsername) {
      await executeQuery('UPDATE Users SET username = ?, updated_at = NOW() WHERE id_user = ?', [newUsername, id_user]);
    }

    if (newPassword) {
      await executeQuery('UPDATE Users SET password = ?, updated_at = NOW() WHERE id_user = ?', [newPassword, id_user]);
    }
...
app.get('/herbs/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const herb = await executeQuery('SELECT * FROM Herbs WHERE id_herbs = ?', [id]);
...
app.post('/favorites', async (req, res) => {
  try {
    ...
    await executeQuery('INSERT INTO Scans (id_user, id_herbs, scan_date) VALUES (?, ?, NOW())', [
      id_user,
      id_herbs,
    ]);
    res.status(201).json({ message: 'Herb added to favorites!' });
  } catch (err) {
    res.status(500).json({ message: 'Error adding herb to favorites', error: err.message });
  }
});
...
app.post('/favorites/remove', async (req, res) => {
  try {
    const { id_user, id_herbs } = req.body;
    await executeQuery('DELETE FROM Scans WHERE id_user = ? AND id_herbs = ?', [id_user, id_herbs]);
    res.json({ message: 'Herb removed from favorites!' });
  } catch (err) {
    res.status(500).json({ message: 'Error removing herb from favorites', error: err.message });
  }
});
